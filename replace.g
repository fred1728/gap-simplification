...
Replace:=function(L,e,n,k)
local perm, i, j, d, l, topdim, bndnk1, cnt, orbits, bool, StabU, sign, Elts, g, c, cell, pos,
newbnddup, newbnd, bnd, bndcell, h, instar, candidates, cellstar, incell, Boundary, 
Stab, Dim, action, oldpos, oldindex, RemapBoundary, oldi, signs,
kstar, ReduceTermOld, NormalizeBoundaryOld;
...
for j in [1..L!.dimension(n+k+1)] do
    ...
    for cell in L!.boundary(n+k+1,j) do
        if AbsoluteValue(cell[1]) in orbits then
            if bool then
                Add(bndnk1[j],[SignInt(cell[1])*topdim,cell[2]]);
                bool:=false;
            fi;
        else
...
Stab:=function(d,i)
    if d<n or d>n+k+1 then
        ...
        return L!.stabilizer(d,Position(perm[d-n+1],i));
    elif d=n+k then
        if i<topdim then
            return L!.stabilizer(d,Position(perm[d-n+1],i));
        elif i=topdim then
...
local g, bndcell, bndcell2, Stabbnd, g1, g2, h, eps1, eps2, eps3, found, oldb;
    ...
    if d = n+k and i = Dim(d) then
        ...
        Stabbnd:=L!.stabilizer(n+k-1,oldb);
        g1:=L!.elts[bndcell[2]];
        found:=false;
        for bndcell2 in Filtered(newbnd,p->AbsoluteValue(p[1])=AbsoluteValue(bndcell[1])) do    
            ...
            if h in Stabbnd then
                eps1:=L!.action(n+k-1,oldb,pos(h));
                eps2:=SignInt(bndcell[1])*SignInt(bndcell2[1]);
                eps3:=eps1*eps2;
                found:=true;
                break;
            fi;
